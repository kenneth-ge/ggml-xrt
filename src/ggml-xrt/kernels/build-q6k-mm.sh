...
#
# Build Q6_K on-chip-dequant FUSED tiled-matmul xclbins (aie2/Phoenix). Serves prefill
# (M tokens) and padded decode (M=1 -> m). C[M,N] += A_act[M,K] . dequant(W_q4K).
#
#   Usage: ./build-q6k-mm.sh [<out_subdir> <K> <N> [<K> <N> ...]]
#     no args -> Qwen3-1.7B Q6_K weight shapes into prebuilt/.
#   K must be a multiple of 256 (q6_K superblock). N must be divisible by n (=32).
#
# Host REPACK contract (== mv_q6k.cc): ONE buffer row-major [N][K/256][212]. Per 256-elem
# superblock per output channel, 212 B = ql[128] + qh[64] + scales[16] int8 + f32 d (near-copy
# of ggml block_q6_K). A bf16 [M,K]; C f32 [M,N]; weight NOT transposed.
# ABI: kernel(op=3, instr@grp1, ninstr, A@grp3, B@grp4, C@grp5).
#
# L1 budget (Phoenix core, 64 KB): 2*(m*k*2)[A] + 2*(n*212)[Bq] + 2*(m*n*4)[C]
...
W="$(mktemp -d)"; cd "$W"
build_core() {  # $1=m $2=n  -> mm_q6k.o compiled for these tile dims
...
try_shape() {  # $1=K $2=N $3=m $4=n $5=M $6=serialize(0/1)  -> 0 on success
  ...
  build_core "$mm" "$nn"
  python "${here}/mm_q6k.py" --dev npu -M "$SM" -K "$K" -N "$N" -m "$mm" -n "$nn" $ser_flag > aie.mlir 2>/dev/null || return 1
...
for ((i = 0; i + 1 < ${#SHAPES[@]}; i += 2)); do
  ...
  # stream fine). Fix: keep M=32 but SERIALIZE m-tiles (dma_wait between them, so
  # each B stream drains before the next) for shapes whose per-m-tile B stream
  # exceeds ~8 MB. Smaller shapes keep the faster ping-pong path unchanged.
  bstream=$(( N * (K / 256) * REC ))
  SM="$M"; SER=0
  ...
  [ "$ok" -eq 1 ] || echo "FAIL q6_K mm ${K}x${N} (all cfgs)"
done
echo "done -> ${DST}"
