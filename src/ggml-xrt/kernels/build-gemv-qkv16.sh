#!/bin/bash
# 16-CORE FUSED q/k/v decode gemv (aie2/Phoenix): one dispatch for q_proj+k_proj+v_proj (shared
# activation) using ALL 16 compute tiles (4 cols x 4 rows), unlike the 4-core gemv_qkv_fused which
# measured a compute regression. Uses the NOSCRATCH cores (mv_q4k_noscratch.cc / mv_q6k_noscratch.cc
# compiled into the mv_q4k.o / mv_q6k.o names the generator links) - the current fast decode kernels.
...
W="$(mktemp -d)"; cd "$W"
# compile the NOSCRATCH cores into the .o names the qkv16 generator links
"${PEANO_INSTALL_DIR}/bin/clang++" -O2 -std=c++20 --target=aie2-none-unknown-elf -Wno-parentheses -Wno-attributes -Wno-macro-redefined -Wno-empty-body -Wno-missing-template-arg-list-after-template-kw -DNDEBUG -DDIM_M="$MM" -DDIM_K=256 -I "${MLIR_AIE_SRC}/aie_kernels/aie2" -I "${MLIR_AIE_INSTALL}/include" -c "${here}/aie2/mv_q4k_noscratch.cc" -o mv_q4k.o
"${PEANO_INSTALL_DIR}/bin/clang++" -O2 -std=c++20 --target=aie2-none-unknown-elf -Wno-parentheses -Wno-attributes -Wno-macro-redefined -Wno-empty-body -Wno-missing-template-arg-list-after-template-kw -DNDEBUG -DDIM_M="$MM" -DDIM_K=256 -I "${MLIR_AIE_SRC}/aie_kernels/aie2" -I "${MLIR_AIE_INSTALL}/include" -c "${here}/aie2/mv_q6k_noscratch.cc" -o mv_q6k.o
python "${here}/gemv_qkv16.py" --dev npu -K "$K" --Nq "$NQ" --Nkv "$NKV" -m "$MM" --rows 4 --qk q4k --v q6k > aie.mlir 2>err.txt || { echo "FAIL gen"; sed -n '1,40p' err.txt; exit 1; }
if aiecc.py --aie-generate-xclbin --no-compile-host --no-xchesscc --no-xbridge --peano "${PEANO_INSTALL_DIR}" \
  ...
  cp g.xclbin "${DST}/gemv_qkv16_${K}.xclbin"; cp gi.bin "${DST}/gemv_qkv16_${K}_insts.bin"
  echo "OK qkv16 (noscratch) K=${K} Nq=${NQ} Nkv=${NKV} -> bench/gemv_qkv16_${K}.xclbin"
else echo "FAIL qkv16 build"; sed -n '1,60p' aiecc.log; exit 1; fi
