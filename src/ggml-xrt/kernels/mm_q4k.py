...
#
# Q4_K on-chip-dequant FUSED tiled matmul (prefill AND padded-decode) for aie2/Phoenix.
#   C[M,N] += A_act[M,K] . dequant(W_q4K)      C f32, A bf16, W packed q4_K.
# Single compute tile. B (weight) is delivered RAW (no memB transform); the core dequants
# each 148-byte q4_K superblock record and scatters bf16 into a design-owned L1 `aie.buffer`
...
# whole point of this design vs. mm_q4k.cc alone.)
#
# k-tile == 256 == one q4_K superblock per weight column (scales never split).
# Weight repack (host, see build-q4k-mm.sh / mv_q4k.cc): ONE buffer row-major
#   [N][K/256][148] : qs[128] + scales[12] + f32 d + f32 dmin  (field reorder of block_q4_K).
...
REC = 148  # q4_K superblock record: qs[128] + scales[12] + f32 d + f32 dmin
QK = 256   # q4_K superblock / forced k-tile
...
def my_matmul_q4k(dev, M, K, N, m, n, serialize=False):
    ...
    assert K % k == 0, "K must be a multiple of 256 (q4_K superblock)"
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            # matmul_q4k_f32(qB, A, Bl1, C)  (see aie2/mm_q4k.cc)
            matmul = external_func(
                "matmul_q4k_f32", inputs=[b_ty, a_ty, bl1_ty, c_ty]
            )
            ...
            # Input B: RAW packed q4_K, NO transform (core dequants+scatters into Bl1).
            ...
            Bl1 = buffer(compute_tile, bl1_ty, "Bl1_q4k")

            @core(compute_tile, "mm_q4k.o", stack_size=0xD00)
            def core_body():
                ...
                for tile_row_block in range(ceildiv(M_div_m, rows_per_block)):
                    for pingpong in [0, 1]:
                        ...
                        for tile_row in range(num_tile_rows):
                            ...
                            # RAW q4_K gather: per (N-tile, k-superblock) deliver n records.
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(prog="ggml-xrt Q4_K fused dequant matmul design")
    ...
    my_matmul_q4k(a.dev, a.M, a.K, a.N, a.m, a.n, a.serialize_mtiles)
