...
#   mv_sv.o        : bf16->f32 scalar matvec (aie2/mv.cc, -DDIM_M=m -DDIM_K=n_kv);
...
# ---- shapes (single head, n_kv bucket) -------------------------------------
N_KV="${N_KV:-512}"          # KV cache length bucket
...
# --- Core A: mv_qkt.o (bf16->bf16, DIM_M=m, DIM_K=head_dim) -----------------
# shellcheck disable=SC2086
"${CLANG}" $CFLAGS -DDIM_M="${M_TILE}" -DDIM_K="${HEAD_DIM}" \
  -I "${AK}" -I "${MLIR_AIE_INSTALL}/include" \
  -c "${here}/aie2/mv_bf16out.cc" -o mv_qkt.o

# --- Core C: mv_sv.o (bf16->f32, DIM_M=m, DIM_K=n_kv) -----------------------
# shellcheck disable=SC2086
"${CLANG}" $CFLAGS -DDIM_M="${M_TILE}" -DDIM_K="${N_KV}" \
  -I "${AK}" -I "${MLIR_AIE_INSTALL}/include" \
  -c "${here}/aie2/mv.cc" -o mv_sv.o

# --- Core B: softmax_ext.o (+ exp LUT partial-link) ------------------------
...
python "${here}/attn_chain.py" -d npu --n_kv "${N_KV}" --head_dim "${HEAD_DIM}" -m "${M_TILE}" \
  > aie_chain.mlir 2>err_chain.txt || { echo "FAIL gen chain"; sed -n '1,40p' err_chain.txt; exit 1; }
...
if aiecc.py --aie-generate-xclbin --no-compile-host --no-xchesscc --no-xbridge \
  ...
  echo "FAIL attn_chain overlay build"; tail -50 aiecc_chain.log; exit 1
fi

echo "DONE -> ${DST}"
ls -la "${DST}"/attn_chain_${N_KV}* 2>/dev/null
