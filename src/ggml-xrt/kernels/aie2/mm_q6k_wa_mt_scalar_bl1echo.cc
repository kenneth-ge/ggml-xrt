...
extern "C" {
// GOLDEN-REFERENCE bl1echo: PROVEN scalar col->Bl1 scatter, then echo Bl1 -> C (skip mmul). The
// re-vectorized scatters (vec3) must byte-match THIS host C. host C[tok,ch]==(float)Bl1[tok*DIM_N+ch].
void matmul_q6k_f32(uint8_t *qB, bfloat16 *A, bfloat16 *Bl1, float *C) {
  (void)A;
  constexpr int s = 8, t = 4, r = 4;
  bfloat16 col[DIM_K];
