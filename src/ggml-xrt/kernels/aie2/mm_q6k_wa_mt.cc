//===- mm_q6k_wa_mt.cc (aie2 / Phoenix) -----------------------*- C++ -*-===//
//
// Q6_K 16-core whole_array verify core — MEMTILE-A variant (A delivered pre-tiled in the mmul A
// sub-tile layout by the memtile DMA transform, so NO on-core A re-tile / Al1 buffer). This is
// the L1-lean core for DIM_M=32 (where the on-core-A-retile variant mm_q6k_wa.cc needs
// 2x16KB for A and overflows the 64KB tile). Everything else is identical to mm_q6k_wa.cc:
// vectorized deq_q6k_vec + VECTORIZED Bl1 scatter (concat+transpose) + matmul_vectorized_4x4.
//
// DIM_M=32 => rowA = DIM_M/4 = 8 => the matmul_vectorized_4x4 outer `z += 4` loop runs 2 iters,
// satisfying AIE_LOOP_MIN_ITERATION_COUNT(2) (DIM_M=16 -> rowA=4 -> 1 iter VIOLATED the hint =>
// dropped-store UB => the exact-zeros bug). Host pads the draft-token count up to 32.
//
...
// mm_q6k_vecdq.cc — proven vectorized dequant; col[] is the natural deq_q6k order.)
...
extern "C" {
// qB: DIM_N superblock records. A: [DIM_M,DIM_K] bf16 ALREADY in the mmul A sub-tile layout
// (delivered by the memtile DMA transform). Bl1: design L1 scratch (DIM_K*DIM_N bf16).
// C: [DIM_M,DIM_N] f32.
void matmul_q6k_f32(uint8_t *qB, bfloat16 *A, bfloat16 *Bl1, float *C) {
  constexpr int s = 8, t = 4;
  constexpr int NT = DIM_N / t;      // n-tile count
  constexpr int KS = DIM_K / s;      // k-tile count
  ...
  for (int nt = 0; nt < NT; nt++) {
    ...
    // VECTORIZED scatter into the mmul B sub-tile layout (no scalar loop).
