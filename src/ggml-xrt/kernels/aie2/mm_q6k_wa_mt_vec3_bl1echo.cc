...
//
// RE-VECTORIZED scatter bl1echo of attempt #3 on the PROVEN scalar baseline (memtile-A, DIM_M=32). NATIVE
...
extern "C" {
// bl1echo of vec3: run the SAME native-32 interleave scatter, echo Bl1 -> C (skip mmul).
// host C[tok,ch] == (float) Bl1[tok*DIM_N + ch]; MUST byte-match the scalar-scatter Bl1
// (mm_verify_wa_split_bl1echo) before trusting mm_verify_wa_vec3.
