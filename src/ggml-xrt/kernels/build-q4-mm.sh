...
try_shape() {  # $1=K $2=N $3=m $4=n  -> 0 on success
  ...
  build_core "$mm" "$nn"
  python "${here}/mm_q4.py" --dev npu -M "$M" -K "$K" -N "$N" -m "$mm" -n "$nn" > aie.mlir 2>/dev/null || return 1
...
for ((i = 0; i + 1 < ${#SHAPES[@]}; i += 2)); do
  ...
  for cfg in "${CFGS[@]}"; do
    ...
    if try_shape "$K" "$N" "$mm" "$nn"; then
      cp q4mm.xclbin    "${DST}/mul_mat_aie2_q4_0_f32_${M}x${K}x${N}_mm.xclbin"
      cp q4mm_insts.bin "${DST}/mul_mat_aie2_q4_0_f32_${M}x${K}x${N}_mm_insts.bin"
      echo "OK q4_0 mm ${M}x${K}x${N} (m=${mm} n=${nn}) -> ${subdir}/"
      ok=1; break
    fi
